...
public class SaveRecipe
{
    public class Handler : EventHandlerAbstract<Request, EntityMessage<int>>
    {
        ...
        public override async Task<IResult<EntityMessage<int>>> HandleAsync(Request request, CancellationToken cancellationToken = default)
        {
            var byId = new RecipesByIdWithCategoriesSpecification(request.Id);

            var maybeRecipe = await _data.Recipes.GetAsync(byId, cancellationToken);

            if (maybeRecipe.HasValue)
            {
                return await maybeRecipe.Value
                    .Tee(r => Transfer(request, r))
                    .TeeAsync(r => _data.Recipes.UpdateAsync(r, cancellationToken))
            ...
            return await new Recipe()
                .Tee(r => Transfer(request, r))
                .TeeAsync(r => _data.Recipes.AddAsync(r, cancellationToken))
                .TeeAsync(r => ManageCategories(request, r, cancellationToken))
        ...
        private async Task ManageCategories(Request request, Recipe recipe, CancellationToken cancellationToken)
        {
            ...
            var categoriesExist = (await _data.Categories.ListAsync(categoriesThatMatchRequestedSpec, cancellationToken))
            ...
            await requested
                ...
                .TeeAsync(r => _data.Categories.AddRangeAsync(r, cancellationToken));
            ...
            await recipe.CategoryRecipe
                ...
                .TeeAsync(r => _data.CategoryRecipes.RemoveRangeAsync(r, cancellationToken));

            // Add relations that don't exist
            await _data.Categories
                .ListAsync(categoriesThatMatchRequestedSpec, cancellationToken)
                ...
                .TeeAsync(r => _data.CategoryRecipes.AddRangeAsync(r, cancellationToken));
        }
    }
